...
function M.tick(task)
  ...
  -- Enemies first.
  local best, best_d
  for _, e in ipairs(vision.filter_visible(c.surface.find_entities_filtered({
    type = ENEMY_TYPES,
    force = game.forces.enemy,
    position = def.anchor,
    radius = task.radius + 8,
  }), c.surface, c.force)) do
    if e.valid then
  ...
  -- Turrets low on ammo.
  for _, t in ipairs(vision.filter_known(c.surface.find_entities_filtered({
    type = "ammo-turret",
    force = c.force,
    position = def.anchor,
    radius = task.radius,
  ...
  -- Damaged structures.
  for _, e in ipairs(vision.filter_known(c.surface.find_entities_filtered({
    force = c.force,
    position = def.anchor,
    radius = task.radius,
  }), c.surface, c.force)) do
