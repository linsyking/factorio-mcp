...
function M.start(task)
  ...
  if task.auto_craft then
    ...
    for _, name in ipairs(names) do
      local base, quality = items.parse(name)
...
function M.tick(task)
  ...
  local item_name, item_quality = items.parse(step.item)
  ...
  if items.count(c, step.item) == 0 then
  ...
  if not can_place then
    return advance(task, false, string.format("can't place %s at (%.1f, %.1f) — %s",
      step.item, step.position.x, step.position.y, placement.explain(c, entity_name, step.position, step.direction)))
  end
