...
local function surface_of()
  return companion.require_companion().surface
end

-- Stations the force knows about (explored/charted chunks only).
local function station_names(surface)
  local names, seen = {}, {}
  local c = companion.require_companion()
  for _, stop in ipairs(vision.filter_known(surface.find_entities_filtered({ type = "train-stop", force = c.force }),
    surface, c.force)) do
