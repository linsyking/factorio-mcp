...
local inventory_map = items.inventory_map
...
local function collect_power(surface, force, origin, radius, structures)
  local poles = vision.filter_known(surface.find_entities_filtered({
    position = origin,
    radius = radius,
    type = "electric-pole",
    force = force,
...
-- Lightweight inventory peek (your own character only).
function M.check_inventory(params)
  local c = companion.require_companion()
  return {
    owner = companion.context(),
...
function M.get_state(params)
  ...
  -- Other agents' characters on this surface (force members are shown on
  -- the map like other players; vision is shared by the force).
  local others = {}
  for _, name in ipairs(companion.names()) do
    if name ~= companion.context() then
      local ent = companion.get(name)
      if ent and ent.surface == surface and ent.force == force then
        others[#others + 1] = {
          name = name,
          position = { x = round1(ent.position.x), y = round1(ent.position.y) },
          distance = round_dist(distance(ent.position, origin)),
        }
      end
    end
  end
  if #others > 0 then out.other_characters = others end
  ...
  local patches = cluster_resources(vision.filter_known(
    surface.find_entities_filtered({ position = origin, radius = radius, type = "resource" }),
    surface, force), origin)
  if #patches > 0 then out.resource_patches = patches end

  out.trees_nearby = #vision.filter_known(
    surface.find_entities_filtered({ position = origin, radius = radius, type = "tree" }), surface, force)

  local status_names = {}
  for name, value in pairs(defines.entity_status) do
    status_names[value] = name
  end
  local structures = structure_groups(vision.filter_known(
    surface.find_entities_filtered({ position = origin, radius = radius, force = force }), surface, force),
    origin, status_names)
  ...
  -- Enemies only count when visible right now (fog of war).
  local enemy_list = vision.filter_visible(surface.find_entities_filtered({
    position = origin,
    radius = radius,
    force = game.forces.enemy,
  }), surface, force)
  local enemies = { spawners = 0 }
  local nearest_d
  for _, e in ipairs(enemy_list) do
    if e.type == "unit-spawner" then enemies.spawners = enemies.spawners + 1 end
    if e.type == "unit" or e.type == "unit-spawner" or e.type == "turret" or e.type == "spider-unit"
      or e.type == "segmented-unit" then
      local d = distance(e.position, origin)
      if not nearest_d or d < nearest_d then nearest_d = d end
    end
  end
  if nearest_d then enemies.nearest_distance = round_dist(nearest_d) end
  out.enemies = enemies
  out.explored_chunks = vision.explored_count(surface, force)
